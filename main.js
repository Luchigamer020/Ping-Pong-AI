...
var paddleX = 10,paddle1Height = 110;
...
function gotPoses(results) {
    if(results.length > 0) {
        rightWristY = results[0]-pose.rigtWrist.y;
...
function draw() {
    if(game_status == "start") {
        ...
        if(scoreRightWrist > 0.2) {
            fill("red");
            stroke("red");
            circle(rightWtistX, rightWristY, 30)    
        }
...
function move() {
    ...
    if(ball.y+ball.r > height || ball.y-ballr <0) {
        ball.dy =- ball.dy;
    }
}
